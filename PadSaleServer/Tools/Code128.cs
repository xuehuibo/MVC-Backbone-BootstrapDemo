...
namespace Tools
{
    ...
    {
        private static DataTable m_Code128 = new DataTable();
        ...
        /// <returns>图形</returns>
        public static Bitmap GetCodeImage(string p_Text, Encode p_Code)
        {
            if (m_Code128.Columns.Count == 0)
            {
                Init();
            }
        ...
        /// <returns>编码</returns>
        private static string GetValue(Encode p_Code, string p_Value, ref int p_SetID)
        {
        ...
        private static Bitmap GetImage(string p_Text)
        {
            ...
            for (int i = 0; i != _Value.Length; i++)
            {
                ...
                if (!((i & 1) == 0))
                {
                    //_Pen = new Pen(new SolidBrush(Color.White), _ValueNumb);
                    _Garphics.FillRectangle(new SolidBrush(Color.White), new Rectangle(_LenEx, 0, _ValueNumb, (int)m_Height));
                }
                else
                {
                    //_Pen = new Pen(new SolidBrush(Color.Black), _ValueNumb);
                    _Garphics.FillRectangle(new SolidBrush(Color.Black), new Rectangle(_LenEx, 0, _ValueNumb, (int)m_Height));
                }
        ...
        private static void GetViewText(Bitmap p_Bitmap, string p_ViewText)
        {
            ...
            _Graphics.FillRectangle(new SolidBrush(Color.White), new Rectangle(0, _StarY, p_Bitmap.Width, (int)_DrawSize.Height));
            _Graphics.DrawString(p_ViewText, m_ValueFont, new SolidBrush(Color.Black), 0, _StarY);
        }
        ...
        internal static Image GetCodeImage(string p)
        {
